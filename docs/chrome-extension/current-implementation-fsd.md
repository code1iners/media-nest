...
- 오디오 모드는 Media Nest API의 오디오 URL 다운로드 계약을 사용한다.
- 비디오 모드는 Media Nest API의 비디오 URL 다운로드 계약을 사용한다.
...
API base URL은 `WXT_MEDIA_NEST_API_BASE_URL` 환경 변수로 정하며 popup에서 사용자 입력을 받지 않는다. 운영 값은 `https://media-nest.codeliners.cc`, 로컬 값은 `http://127.0.0.1:3030`만 사용한다.
...
- API base URL은 `WXT_MEDIA_NEST_API_BASE_URL`을 사용하고 popup UI에서 바꿀 수 없다.
