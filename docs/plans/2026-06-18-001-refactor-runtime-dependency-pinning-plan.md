...
- `package-lock.json`: `youtube-dl-exec` npm 패키지가 `3.0.30`으로 잠겨 있음.
- `src/audio/audio.service.ts`: `FFMPEG_LOCATION`을 `youtube-dl-exec`의 `ffmpegLocation`으로 전달.
- `src/video/video.service.ts`: 비디오 병합에서도 같은 `FFMPEG_LOCATION` 계약 사용.
- `.env.example`: production 기준 `FFMPEG_LOCATION=/usr/bin/ffmpeg`.
- `README.md`: Docker 실행 방법과 런타임 요구사항 문서.
- `docs/current-implementation-fsd.md`, `docs/current-implementation-prd.md`: 현재 실행 환경과 한계 문서.
...
- build stage에서 `npm ci` 전에 `python3`와 필요한 인증서 패키지를 설치한다.
...
**Approach:**
- `npm ci` 실행 전에 `YOUTUBE_DL_HOST`를 특정 `yt-dlp` release asset URL로 지정한다.
- 필요하면 `YOUTUBE_DL_FILENAME`은 기본 `yt-dlp`를 유지해 라이브러리 기본 경로와 충돌하지 않게 한다.
- npm package lock은 그대로 유지하되, 바이너리 버전은 Dockerfile 또는 문서에서 별도 관리 대상으로 명시한다.

**Patterns to follow:**
- `package-lock.json`으로 npm 의존성을 고정하는 현재 방식.
...
**Files:**
- Modify: `README.md`
- Modify: `docs/current-implementation-fsd.md`
- Modify: `docs/current-implementation-prd.md`
...
- Related code: `package-lock.json`
- Related code: `src/audio/audio.service.ts`
- Related code: `src/video/video.service.ts`
- Related docs: `README.md`
- Related docs: `docs/current-implementation-fsd.md`
- Related docs: `docs/current-implementation-prd.md`
