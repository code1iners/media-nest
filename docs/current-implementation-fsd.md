...
- 주요 환경 변수:
  ...
  - `EXPECTED_NODE_MAJOR`, `EXPECTED_YT_DLP_VERSION`, `EXPECTED_FFMPEG_LOCATION`, `EXPECTED_FFMPEG_VERSION`: `npm run verify:runtime` 실행 시 런타임 의존성 기대값으로 사용
...
- 검증 실패 시 `400` 상태를 반환함
- 다운로드 생성 실패 시 `500` 상태와 `Failed generating video file` 또는 전송 실패 메시지를 반환할 수 있음
...
- 검증 실패 시 `400` 상태를 반환함
- 다운로드 생성 실패 시 `500` 상태와 `Error generating audio file` 또는 전송 실패 메시지를 반환할 수 있음
...
- Node.js, ffmpeg, yt-dlp 버전과 실행 파일 존재 여부는 별도 `npm run verify:runtime` 명령으로 확인한다.
