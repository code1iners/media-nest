...
- 응답에는 R2 credential, 임시 파일 경로, 내부 오류 원문을 포함하지 않는다.
...
- 완료 SRT가 만료되면 `displayStatus: "expired"`를 반환한다.
...
`completed` job의 영어 SRT를 attachment stream으로 내려준다.

응답:

- `completed`: `application/x-subrip; charset=utf-8` `Content-Type`, `Content-Disposition` attachment 응답
- `queued`, `extracting_audio`, `transcribing`, `failed`: `404`
- `expired`: `410`
- 알 수 없는 `jobId`: `404`

현재 처리 방식:

- worker는 R2 원본 영상을 임시 파일로 내려받고 ffmpeg로 mono 16kHz wav를 만든다.
- 추출 audio가 local Whisper 처리 보호용 `SUBTITLE_AUDIO_MAX_BYTES`를 넘으면 `AUDIO_TOO_LARGE`로 실패 처리한다.
- worker는 사용자가 준비한 `WHISPER_CLI_PATH`와 선택 모델별 `WHISPER_MODEL_BASE_EN_PATH` 또는 `WHISPER_MODEL_SMALL_EN_PATH`로 `whisper.cpp` CLI를 실행하고 `language=en`, SRT 출력 옵션으로 요청한다.
- 생성된 SRT는 R2 `subtitles/{jobId}/english.srt`에 저장하고 `resultObjectKey`를 기록한다.
- 만료된 subtitle job은 hourly cleanup에서 source 영상과 result SRT를 삭제한 뒤 DB row를 삭제한다.
- 긴 영상 chunking, OCR, 한글 번역, 영상 burn-in은 현재 범위가 아니다.
...
- worker는 한 번에 하나의 다운로드 job 또는 자막 job만 claim한다.
