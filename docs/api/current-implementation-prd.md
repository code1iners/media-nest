...
Subtitles 도메인은 로컬 영상 파일 업로드를 PostgreSQL job으로 생성하고, worker가 영어 SRT를 만든 뒤 R2 asset 기반 파일 다운로드를 제공한다.

- `POST /subtitles/jobs`: `multipart/form-data`의 `file` 필드로 `mp4`, `mov`, `webm` 영상을 받고 `whisperModel`로 `base_en` 또는 `small_en`을 선택해 자막 job을 만든다.
