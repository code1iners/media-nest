...
MyTube Extract web 앱은 사용자가 브라우저에서 YouTube URL을 입력해 다운로드 job을 만들고, 준비 상태를 확인한 뒤 완료된 파일을 받을 수 있게 하는 Vite CSR 앱이다.
...
- `/video` 영상 추출 route
- `/subtitles` 자막 추출 placeholder route
- `/`에서 `/video`로 redirect
...
- 사용자 계정과 작업 이력은 없다.
- 자막 추출 기능은 route placeholder만 제공하며 실제 추출 UI와 API 연결은 없다.
