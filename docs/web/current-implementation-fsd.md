...
- 다운로드 요청 검증과 API 호출 조합은 `src/domain/download-request/download-request.ts`가 담당한다.
...
3. 사용자가 모드별 품질을 선택한다.
4. 앱이 `POST /downloads`로 job을 만든다.
5. 앱이 terminal 상태까지 `GET /downloads/:jobId`를 2500ms 간격으로 polling한다.
6. `completed` 상태가 되면 `downloadUrl`을 API base URL과 결합해 다운로드 링크를 보여준다.
...
- `pnpm --filter web run test`: URL 검증과 다운로드 요청 조합을 검증한다.
